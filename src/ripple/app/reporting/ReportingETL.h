...
private:
    ...
    /// published very rapidly. Monitoring processes will publish ledgers as they
    /// are written. However, to publish a ledger, the monitoring process needs to
    /// read all of the transactions for that ledger from the database. Reading
    /// the transactions from the database requires network calls, which can be
    /// slow. It is imperative however that the monitoring processes keep up with
    /// the writer, else the monitoring processes will not be able to detect if
    /// the writer failed. Therefore, publishing each ledger (which includes
    /// reading all of the transactions from the database) is done from the
    /// application wide asio io_service, and a strand is used to ensure ledgers
    ...
    /// Mechanism for detecting when the network has validated a new ledger. This
    /// class provides a way to wait for a specific ledger to be validated
    NetworkValidatedLedgers networkValidatedLedgers_;

    /// Whether the software is stopping
    std::atomic_bool stopping_ = false;

    /// Used to determine when to write to the database during the initial ledger
    /// download. By default, the software downloads an entire ledger and then writes
    /// to the database. If flushInterval_ is non-zero, the software will write to
    /// the database as new ledger data (SHAMap leaf nodes) arrives. It is not neccesarily more effient
    /// to write the data as it arrives, as different SHAMap leaf nodes share the
    /// same SHAMap inner nodes; flushing prematurely can result in the same SHAMap
    /// inner node being written to the database more than once. It is recommended
    /// to use the default value of 0 for this variable; however, different values
    /// can be experimented with if better performance is desired.
    ...
    /// allows clients to page through a ledger over many RPC calls. GetLedgerData
    /// returns a marker that is used as an offset in a subsequent call. If numMarkers_
    /// is greater than 1, there will be multiple chains of GetLedgerData calls
    /// iterating over different parts of the same ledger in parallel. This can
    /// dramatically speed up the time to download the initial ledger. However,
    /// a higher value for this member variable puts more load on the ETL source.
    size_t numMarkers_ = 2;

    /// Whether the process is in strict read-only mode. In strict read-only mode,
    /// the process will never attempt to become the ETL writer, and will only
    /// publish ledgers as they are written to the database.
    ...
    /// the next ledger validated by the network. If this is set, and the database
    /// is already populated, an error is thrown.
    ...
    /// Download a ledger with specified sequence in full, via GetLedgerData, and
    /// write the data to the databases. This takes several minutes or longer.
    ...
    /// Monitor the network for newly validated ledgers. Also monitor the database
    /// to see if any process is writing those ledgers. This function is called
    /// when the application starts, and will only return when the application
    /// is shutting down. If the software detects the database is empty, this
    /// function will call loadInitialLedger(). If the software detects ledgers are
    /// not being written, this function calls runETLPipeline(). Otherwise, this
    ...
    /// runETLPipeline() or loadInitialLedger(). This function only publishes ledgers
    ...
    /// transaction hashes, corresponding nodestore hashes and affected accounts)
    ...
    /// @param maxAttempts the number of times to attempt to read the ledger from
    /// the database. 1 attempt per second
    ...
    /// This function will continue to pull from the queue until the queue returns
    /// nullptr. This is used during the initial ledger download
...
public:
    ...
    /// Get the number of markers to use during the initial ledger download. This
    /// is equivelent to the degree of parallelism during the initial ledger
    /// download

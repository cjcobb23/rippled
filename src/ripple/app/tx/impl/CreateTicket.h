...
public:
    ...
    //
    // For timing I used my own laptop and a release build with asserts off.
    // I took 20 measurements each of the Payment and TicketCreate
    // transactions and averaged to get my timings.
    //
    // For the example compute-intensive Payment I picked a Discrepancy
    // unit test Payment which has 3 paths.  With all the latest amendments
    // enabled, that Payment::doApply() operation took, on average, 1.25 ms.
    //
    // Then I found that creating 250 Tickets in a single

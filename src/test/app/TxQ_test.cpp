...
public:
    ...
    void testQueueTicket()
    {
        ...
        env(noop(alice),
            ticket::use(tkt1 + 13), fee (23), ter(telCAN_NOT_QUEUE_FULL));
        ...
        env(noop(alice), ticket::use(tkt250 -  3), fee(24), queued);
        env(noop(alice), ticket::use(tkt1   + 17), fee(23), queued);
        env(noop(alice),
            ticket::use(tkt250 -  4), fee(22), ter(telCAN_NOT_QUEUE_FULL));
        env(noop(alice),
            ticket::use(tkt1   + 18), fee(21), ter(telCAN_NOT_QUEUE_FULL));

        checkMetrics(env, 12, 12, 7, 6, 257);

        env.close();
        env.require (owners (alice, 229), tickets (alice, 229));

        // These four ticket-based transactions escaped the queue.
        ...
        env(noop(alice), ticket::use(tkt1   + 17), ter(tefNO_TICKET));

        // But these two ticket-based transactions are in the queue
        ...
        // These four ticket-based transactions also escaped the queue.
        env(noop(alice), ticket::use(tkt250 -  3), ter(tefNO_TICKET));
        ...
        env(noop(alice), ticket::use(tkt1   + 18), fee (26),
        ...
        env(noop(alice), ticket::use(tkt250 -  4), fee (27),
            ter(telCAN_NOT_QUEUE_FEE));
    ...
    void testZeroFeeTxn()
    {
        ...
        // Since bob has entries in his queue he cannot add a blocker.
        env(regkey(bob, alice), fee(0),
            seq(seqBob), ter(telCAN_NOT_QUEUE_BLOCKS));
        checkMetrics(env, 4, 8, 5, 4, 256);
        ...
        // out Bob's last queued transaction.
    ...
    void testMultiTxnPerAccount()
    {
        ...
        env(noop(bob), seq(bobSeq + 5),
            fee(bobFee), ter(terINSUF_FEE_B));
        checkMetrics(env, 10, 12, 7, 6, 256);

        // Replace a middle item in the queue
        // with enough fee to bankrupt bob, and also
        // to use fee averaging to clear out the first
        // six transactions
        bobFee =
            env.le(bob)->getFieldAmount(sfBalance).xrp().drops() - (9 * 10);
        env(noop(bob), seq(bobSeq + 5),
            fee(bobFee));
        checkMetrics(env, 4, 12, 13, 6, 256);

        // Make sure that a queued item cannot be replaced with a
        // higher fee transaction, because the existing queued
        // transactions will eat all of bob's balance.
        bobSeq = env.seq(bob);
        // check the first
        env(noop(bob), seq(bobSeq),
            fee(20), ter(terINSUF_FEE_B));
        // and the last
        env(noop(bob), seq(bobSeq + 3),
            fee(20), ter(terINSUF_FEE_B));

        // Now here's the important part: close the ledger and verify
        // that the rest of the queued transactions succeed, and bob's
        // balance is 0
        env.close();
        checkMetrics(env, 0, 26, 4, 13, 256);
        env.require(balance(alice, XRP(0)));
    }
    ...
    void testBlockersSeq()
    {
        ...
        checkMetrics(env, 0, boost::none, 2, 3, 256);
        {
            ...
            env(fset(alice, asfAccountTxnID), seq(aliceSeq + 0), fee(20),
            ...
            env(signers(alice, 2, { {bob}, {charlie}, {daria} }),
                seq(aliceSeq + 2), fee(20), ter(telCAN_NOT_QUEUE_BLOCKS));
    ...
    void testBlockersTicket()
    {
        ...
        std::uint32_t tkt {env.seq(alice) + 1};
        {
            ...
            // Can't replace either queued transaction with a blocker
            env(fset(alice, asfAccountTxnID), ticket::use(tkt + 1), fee(20),
                ter(telCAN_NOT_QUEUE_BLOCKS));

            env(regkey(alice, bob), ticket::use(tkt + 2), fee(20),
                ter(telCAN_NOT_QUEUE_BLOCKS));

            // Can't append a blocker to the queue.
            env(signers(alice, 2, { {bob}, {charlie}, {daria} }),
                fee(20), ter(telCAN_NOT_QUEUE_BLOCKS));

            env(signers(alice, 2, { {bob}, {charlie}, {daria} }),
                ticket::use(tkt + 0), fee(20), ter(telCAN_NOT_QUEUE_BLOCKS));
        ...
        }
        {
            ...
            env(regkey(alice, bob),
                ticket::use(tkt + 1), fee(20), ter(telCAN_NOT_QUEUE_BLOCKS));

            // However we can _replace_ that lone entry with a blocker.
            env(regkey(alice, bob),
                ticket::use(tkt + 0), fee(20), queued);

            // Now that there's a blocker in the queue we can't append to
            // the queue.
            env(noop(alice), ter(telCAN_NOT_QUEUE_BLOCKED));
            env(noop(alice),
                ticket::use(tkt + 1), ter(telCAN_NOT_QUEUE_BLOCKED));
            ...
            // We can replace the blocker with a different blocker.
            env(signers(alice, 2, { {bob}, {charlie}, {daria} }),
                ticket::use(tkt + 0), fee(26), queued);
    ...
    void testExpirationReplacement()
    {
        ...
        */
        ...
        env(fset(alice, asfAccountTxnID),
            seq(aliceSeq), ter(telCAN_NOT_QUEUE_BLOCKS));
        checkMetrics(env, 2, 40, 5, 4, 256);
    ...
    void testAccountInfo()
    {
        ...
        envs(noop(alice), fee(100), seq(none),
    ...
    testInLedgerSeq()
    {
        ...
        env.app().openLedger().modify(
            [&](OpenView& view, beast::Journal j)
        {
            auto const tx = env.jt(noop(alice), seq(aliceSeq),
                openLedgerFee(env));
    ...
    testInLedgerTicket()
    {
        ...
        env.app().openLedger().modify(
            [&](OpenView& view, beast::Journal j)
        {
            auto const tx = env.jt(noop(alice), ticket::use(tktSeq0 + 1),
                openLedgerFee(env));
            auto const result = ripple::apply(
                env.app(), view, *tx.stx,
                tapUNLIMITED, j);
        ...
        // We should still be able to force all of the queued transactions
        // into the ledger by adding one more queued transaction that pays
        // enough so fee averaging kicks in.  Paying the openLedgerFee
        // is not sufficient.

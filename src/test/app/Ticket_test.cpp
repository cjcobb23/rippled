...
class Ticket_test : public beast::unit_test::suite
{
    ...
    void checkTicketConsumeMeta (test::jtx::Env& env)
    {
        ...
        std::string const account {tx[sfAccount.jsonName].asString()};
        if (! tx.isMember (sfTicketSequence.jsonName))
        {
            fail ("Not metadata for a ticket consuming transaction.",
                __FILE__, __LINE__);
            return;
        }
        ...
        Json::Value const& metadata {env.meta()->getJson (JsonOptions::none)};
        if (! metadata.isMember (sfTransactionResult.jsonName))
        {
            fail ("Metadata is missing TransactionResult.",
                __FILE__, __LINE__);
            return;
        }
        {
            ...
            {
                fail (transactionResult + " neither tesSUCCESS nor tec",
                    __FILE__, __LINE__);
                return;
            }
        }
        ...
        for (Json::Value const& node : metadata[sfAffectedNodes.fieldName])
        {
            if (node.isMember (sfModifiedNode.jsonName))
            {
                        ...
                        [sfAccount.jsonName].asString() == account)
                {
                    ...
                    // be gone.
                    if (! modified[sfPreviousFields.jsonName].isMember (
                        sfTicketCount.jsonName))
                    {
                        fail ("AccountRoot previous is missing TicketCount",
                            __FILE__, __LINE__);
                        return;
                    }
                    std::uint32_t const prevTicketCount {
                        modified[sfPreviousFields.jsonName]
                            [sfTicketCount.jsonName].asUInt()};

                    BEAST_EXPECT (prevTicketCount > 0);
                    if (prevTicketCount == 1)
                        BEAST_EXPECT (! modified[
                            sfFinalFields.jsonName].isMember (
                                sfTicketCount.jsonName));
                    else
                        BEAST_EXPECT (
                            modified[sfFinalFields.jsonName].isMember (
                                sfTicketCount.jsonName) &&
                            modified[sfFinalFields.jsonName]
                                [sfTicketCount.jsonName].asUInt() ==
                                    prevTicketCount - 1);
            ...
            else if (node.isMember (sfDeletedNode.jsonName))
            {
                ...
                if (entryType == jss::Ticket)
                {
                    ...
                    // Verify the deleted ticket has the right TicketSequence.
                    BEAST_EXPECT (deleted[sfFinalFields.jsonName]
                        [sfTicketSequence.jsonName].asUInt() == ticketSeq);
    ...
    void testTicketNotEnabled ()
    {
        ...
        env(fset (env.master, asfDisableMaster),
            ticket::use(ticketSeq++), ter (tecNO_ALTERNATIVE_KEY));
        checkTicketConsumeMeta (env);
        env.close();
        env.require (owners (env.master, 0), tickets (env.master, 0));
    }
    ...
    void testTicketCreatePreclaimFail ()
    {
        ...
        }
        {
            ...
            env (ticket::create (alice, 2),
                ticket::use (ticketSeq + 1), ter (tecDIR_FULL));
            env.close();
        ...
        }
        {
            ...
            // threshold.
            env (ticket::create (alice, 250),
                ticket::use (ticketSeq_AB + 0), ter (tecDIR_FULL));
            env.close();
    ...
    void testTicketInsufficientReserve ()
    {
        ...
        env (pay (env.master, alice, env.current()->fees().accountReserve (1) -
            env.balance (alice)));
        env.close();
...
//        BEAST_EXPECT (ticketSeq_F + 1 == env.seq (alice));
    ...
    void testTransactionDatabaseWithTickets()
    {
        ...
        auto getTxID = [&env, this] () -> uint256
        {
            ...
            if (!tx)
            {
                this->fail ("Transaction not found");

...
struct PayChan_test : public beast::unit_test::suite
{
    ...
    testSimple ()
    {
        ...
        env (fund (alice,
            channel (alice, "noAccount", env.seq(alice) - 1), XRP (1000)),
            ter (tecNO_ENTRY));
        // not enough funds
    ...
    testUsingTickets()
    {
        ...
        }
        {
            ...
            env (claim (bob, chan, reqBal, authAmt, Slice (sig), alice.pk ()),
                ticket::use (bobTicketSeq++), ter (tecUNFUNDED_PAYMENT));
        ...
        }
        {
            ...
            auto const sig =
                signClaimAuth (alice.pk (), alice.sk (), chan, authAmt);
            env (claim (bob, chan, reqAmt, authAmt, Slice (sig), alice.pk ()),
                ticket::use (bobTicketSeq), ter (temBAD_AMOUNT));
        ...
        env (fund (bob, chan, XRP (1000)),
            ticket::use (bobTicketSeq++), ter (tecNO_PERMISSION));
...

        {
            ...
            auto const preBob = env.balance (bob);
            env (claim (bob, chan),
                txflags (tfClose), ticket::use (bobTicketSeq++));
